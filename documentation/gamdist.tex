...
First described by \cite{HT:86}, Generalized Additive Models extend Generalized Linear Models, which themselves extend linear models. Let us therefore begin with the linear model. Linear models are concerned with the conditional distribution of a response $Y$ given one or more features, $X=x$. The most common type of linear model assumes this conditional distribution is Gaussian, with mean $\mu(x) = \nu(x)^T \beta$ and variance $\sigma^2$. In other words, $Y \given X = x \sim \normal (\mu(x), \sigma^2)$. The mean of this distribution is permitted to depend on the features, but the variance is constant. Moreover, the dependence of the mean on the features is parametric: $\nu(x)$ is chosen by the statistician in advance. 

Often, $\nu(x) = x$ (sometimes we augment $\nu(x)$ with a constant to capture an affine term), but the model can be extended by adding polynomial  or otherwise parametric functions of the features. For example, we might choose $\nu(x) = \begin{bmatrix}x_1 & x_2 & x_1^2 & x_1 \cdot x_2 & x_2^2 \end{bmatrix}^T$ to capture first and second order terms of a model with two features. As another example, we might define $\nu(x) = \begin{bmatrix} 1 & \log(x_1) \end{bmatrix}^T$ to capture an affine relationship between the mean and the logarithm of $x_1$. These models are nonlinear in the features, but linear in the parameters $\beta$.
...
Suppose we have $n$ observations of the form $\{ (x^{(i)}, \,y^{(i)} ) \}_{i=1, \ldots, n}$. We assume these observations are independent and drawn from the same (unknown) joint distribution. Under the linear model, $Y \given X = x^{(i)} \sim \normal (\mu(x^{(i)}), \sigma^2)$. The likelihood of a particular observation is
\begin{displaymath}
