...
We can test for \textit{serial correlation} if the order of observations is physically meaningful. In the absence of correlation, a residual with positive sign is equally likely to be followed by a residual with positive or negative sign, which can easily be examined graphically. Another approach is Durbin and Watson's $d$-test, which checks for a first order autoregressive model for the residuals: $\hat{\epsilon}^{(i)} = \rho \hat{\epsilon}^{(i-1)} + \delta^{(i)}$, where $\delta^{(i)}$. Let
\begin{displaymath}
   D = \frac{\sum_{i=2}^n (\hat{\epsilon}^{(i)} - \hat{\epsilon}^{(i-1)})^2}{\sum_{i=1}^n (\hat{\epsilon}^{(i)})^2}.
\end{displaymath}
This statistic will always be between 0 and 4. If it is close to 0, that is evidence of $\rho > 0$. If it is close to 4, that is evidence of $\rho < 0$. If it is close to 2, there is no evidence of serial correlation. (How do we calculate confidence intervals on $\rho$? How do we compute p-value? Power?)
...
\end{displaymath}
where $\hat{\mathcal{I}}_{\textrm{augmented}}^{-1}$ is the $k \times k$ submatrix of $\hat{\mathcal{I}}^{-1}$ corresponding to the augmented entries of $\nu$. The p-value is $\Phi_{F_{k, n-p-k}}(-| T_C|) + (1 - \Phi_{F_{k, n-p-k}}(|T_C|))$. If this p-value is small, that constitutes evidence that at least one of the observations under consideration is an outlier.
